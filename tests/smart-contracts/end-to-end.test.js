...
(0, globals_1.describe)('End-to-End Smart Contract Workflow Tests', () => {
    let context;
    ...
    (0, globals_1.describe)('Complete Intent Execution Workflow', () => {
        (0, globals_1.it)('should execute complete intent workflow successfully', async () => {
            ...
            const quote = (0, setup_1.createMockQuote)(solver.accountId, intent.id);
            await solver.call(context.verifierContract, 'submit_quote', { quote }, {
            ...
            await user.call(context.verifierContract, 'execute_intent', {
                intent_id: intent.id,
            ...
            (0, globals_1.expect)(performance.total_executions).toBeGreaterThan(0);
        });
        (0, globals_1.it)('should handle multiple competing quotes', async () => {
            ...
            const quotes = [
                { ...(0, setup_1.createMockQuote)(solver1.accountId, intent.id), amount_out: '9700000' },
                { ...(0, setup_1.createMockQuote)(solver2.accountId, intent.id), amount_out: '9800000' },
                { ...(0, setup_1.createMockQuote)(solver3.accountId, intent.id), amount_out: '9750000' },
            ];
            for (const quote of quotes) {
                const solverAccount = quote.solver_id === solver1.accountId ? solver1 :
                    quote.solver_id === solver2.accountId ? solver2 : solver3;
                await solverAccount.call(context.verifierContract, 'submit_quote', { quote }, {
                    attachedDeposit: near_workspaces_1.NEAR.parse('0.005').toString(),
                    gas: '200000000000000',
                });
            }
            ...
            (0, globals_1.expect)(allQuotes.length).toBe(3);
            ...
            (0, globals_1.expect)(status.solver_id).toBe(solver2.accountId);
        });
        (0, globals_1.it)('should handle intent cancellation workflow', async () => {
            ...
            // User cancels intent
            await user.call(context.verifierContract, 'cancel_intent', { intent_id: intent.id }, { gas: '200000000000000' });
            // Verify cancellation
            const status = await context.intentManagerContract.view('get_intent_status', {
                intent_id: intent.id,
            });
            ...
            try {
                ...
                (0, globals_1.expect)(true).toBe(false); // Should not accept quotes for cancelled intent
            }
            catch (error) {
    ...
    (0, globals_1.describe)('Error Handling and Edge Cases', () => {
        (0, globals_1.it)('should handle insufficient deposit for intent submission', async () => {
            const user = context.testUsers.alice;
            const intent = (0, setup_1.createMockIntent)(user.accountId);
            try {
                await user.call(context.verifierContract, 'submit_intent', { intent }, {
        ...
        (0, globals_1.it)('should handle quote submission after intent expiry', async () => {
            const user = context.testUsers.alice;
            const solver = context.testSolvers.solver1;
            ...
            await user.call(context.verifierContract, 'submit_intent', { intent: shortLivedIntent }, {
                attachedDeposit: near_workspaces_1.NEAR.parse('0.01').toString(),
                gas: '300000000000000',
            });
            // Wait for expiry
            ...
            const quote = (0, setup_1.createMockQuote)(solver.accountId, shortLivedIntent.id);
            try {
                await solver.call(context.verifierContract, 'submit_quote', { quote }, {
                    attachedDeposit: near_workspaces_1.NEAR.parse('0.005').toString(),
                    gas: '200000000000000',
        ...
        (0, globals_1.it)('should handle execution by non-owner', async () => {
            const user = context.testUsers.alice;
            ...
            try {
                await maliciousUser.call(context.verifierContract, 'execute_intent', {
                    intent_id: intent.id,
                    quote_id: solver.accountId,
                }, {
                    attachedDeposit: near_workspaces_1.NEAR.parse('0.1').toString(),
    ...
    (0, globals_1.describe)('Performance and Scalability', () => {
        (0, globals_1.it)('should handle multiple concurrent intents', async () => {
            const users = [context.testUsers.alice, context.testUsers.bob, context.testUsers.charlie];
            const intents = users.map(user => (0, setup_1.createMockIntent)(user.accountId));
            // Submit multiple intents concurrently
            await Promise.all(intents.map((intent, index) => users[index].call(context.verifierContract, 'submit_intent', { intent }, {
                attachedDeposit: near_workspaces_1.NEAR.parse('0.01').toString(),
                gas: '300000000000000',
            })));
        ...
        (0, globals_1.it)('should maintain performance with many quotes', async () => {
            const user = context.testUsers.alice;
            const intent = (0, setup_1.createMockIntent)(user.accountId);
            await user.call(context.verifierContract, 'submit_intent', { intent }, {
                attachedDeposit: near_workspaces_1.NEAR.parse('0.01').toString(),
            ...
            for (let i = 0; i < 3; i++) {
                for (const solver of solvers) {
                    const quote = {
                        ...(0, setup_1.createMockQuote)(solver.accountId, intent.id),
            ...
            (0, globals_1.expect)(quotes.length).toBe(9); // 3 solvers × 3 quotes each
        });
    });
});
